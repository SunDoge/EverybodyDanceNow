...
class LocalEnhancer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3, n_blocks_global=9,
        ...
        model_global = [model_global[i]
                        for i in range(len(list(model_global))-3)]
...
class GlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        downsample_end_index = len(model)
        ...
        resnet_blocks_end_index = len(model)
        ...
        upsample_end_index = len(model)

        self.model = nn.Sequential(*model)

        self._downsample_end_index = downsample_end_index
        self._resnet_blocks_end_index = resnet_blocks_end_index
        self._upsample_end_index = upsample_end_index

    def forward(self, input):
        return self.model(input)

    def forward_with_flow(self, input):

        # 这里认为只有downsample部分是encoder
        encoder = self.model[:self._downsample_end_index]
        ...
        if not hasattr(self, 'prev_label'):
            """
            input里面有两张图，一张是label，一张是上一次的输出
            我们记录下label，用于下次计算
            """
            label, _prev_out = input.chunk(2, dim=1)
            ...
            如果有上一张label，就计算光流
            """
            from util.optical_flow import get_flow_with_size, flow_warp
            from util.util import tensor2im
            
            label, _prev_out = input.chunk(2, dim=1)

            printonce('feat:', feat.shape[-2:])
            # 拿到H W, opencv的size貌似是反向的
            flow, reliable = get_flow_with_size(
                tensor2im(self.prev_label[0]), tensor2im(label[0]), self.dis, size=feat.shape[-2:][::-1])

            warp_feat = flow_warp(self.prev_feat, flow)

            reliable = reliable * 0.75

            feat = warp_feat * reliable + feat * (1 - reliable)

            # 存下更新后的feat，Eval_video_smooth.py就是这样写的
            self.prev_label = label
            self.prev_feat = feat

        # 这里认为downsample下面都是decoder
        decoder = self.model[self._downsample_end_index:]
        return decoder(feat)
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, getIntermFeat=False, addname=''):
        ...
        if getIntermFeat:
            for n in range(len(sequence)):
                setattr(self, addname + 'model'+str(n),
